...
Commands:
  ...
  check             Check DISPLAY, GPU, Isaac Gym, PyTorch, and X1 task shape.
  ...
  tensorboard       Start TensorBoard for logs/x1_dh_stand.
...
Examples:
  ...
  TASK=x1_dh_stand NUM_ENVS=10 bash ops/gradmotion/gui-desktop-train.sh gui-hold-focused
...
print("action_scale:", env_cfg.control.action_scale)
PY
}
...
default_run_name() {
  local suffix="$1"
  date +"x1_${suffix}_%Y%m%d_%H%M%S"
}
...
run_tensorboard() {
  ...
  tensorboard --logdir logs/x1_dh_stand --host 0.0.0.0 --port "${TENSORBOARD_PORT}"
}
